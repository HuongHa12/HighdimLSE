...
from utils.func_helper import model_predict, generate_hlevel_func, save_dict, makedirs
...
if __name__ == '__main__':
    ...
    # Load the function and the corresponding statistics
    myfunction, max_func, _, sign = generate_hlevel_func(bm_function)
    func = myfunction.func
    bounds_all = np.asarray(myfunction.bounds)

    # Generate the discrete function domain and compute the true h_thrs
    np.random.seed(2020)
    X_MC = [np.random.uniform(x[0], x[1], size=10000*myfunction.input_dim) for x in bounds_all]
    X_MC = np.asarray(X_MC)
    X_MC = X_MC.T
    Y_MC= np.abs(func(X_MC))
    prob_true_all = []
    ...
    # Generate inputs/outputs data and generate the threshold level
    # based on the maximum of current observations
    seed = n_exp
    np.random.seed(seed)
    ...
    Y_temp = np.abs(func(X_init))
    Y_init = Y_temp

    # Fit the surrogate model: GP and BNN
    # Be careful with the fitting methodology (it can be stuck in local minima)
    # Multi-start needs to be used
    ...
    # Save the statistics of all the BNN-based LSE methods
    # DO NOT save the statistics of GP-LSE now because it depends on
    # some future computations - NOTE this doesn't affect the comparison bw methods
    eps = 1e-5
    prob_est_iter = []
    F1score_iter = []
    Prec_iter = []
    ...
    for n_iter in range(1, max_iter+1):
        ...
        if (acq_model == 'ImpHLSE'):
            ...
            # Warming up with a lot of random points
            x_tries = X_MC.copy()
...
#                    print('Duplicate samples, pick a sample with the next largest value!')
                    cnt += 1
                    idx_max = ys_sort_idx[cnt]
                    x_max = x_tries[idx_max]
        ...
        # Later to load results, one can use the load_dict() function from utils.func_helper
        print('Save results ...')
