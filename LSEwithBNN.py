...
if __name__ == '__main__':
    ...
    # Load the function
    myfunction, _, h_thrs_list, sign = generate_hlevel_func(bm_function)
    func = myfunction.func
    bounds_all = np.asarray(myfunction.bounds)
    h_thrs = h_thrs_list[h_idx]

    # Compute the true probability function
    np.random.seed(2020)
    ...
    # Fit the surrogate model for the BNN-based methods
    ...
    # Save the interested statistics (F1 score, Precision, Recall) of some methods
    prob_est_iter = []
    F1score_iter = []
    Prec_iter = []
    Rec_iter = []
